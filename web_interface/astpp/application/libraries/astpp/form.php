...
function build_form($fields_array,$values) {
	    ...
	    if(count($fields_array) > 1)
            ...
            $form_contents.= '<ul>';
             ...
             if ($i == 1 || $i == 3) {
                   $form_contents.= form_fieldset($fieldset_key, array('style' => 'margin-left:0px;margin-top:10px;font-weight:bold;margin-right:90px;'));
                }else{
                    $form_contents.= form_fieldset($fieldset_key, array('style' => 'margin-left:90px;margin-top:10px;font-weight:bold;margin-right:20px;'));
                }  
            
            
            
//            $form_contents.= form_fieldset($fieldset_key, array('style' => 'margin-left:50px;font-weight:bold;margin-right:50px;'));
            ...
            foreach ($form_fileds as $fieldkey => $fieldvalue) {
                 ...
                 if($fieldvalue[1] == 'HIDDEN')
                {
                    ...
                    if(is_array($fieldvalue[1]) || (is_array($fieldvalue[2]) && isset($fieldvalue[2]['hidden']))){
                        $form_contents.= form_label($fieldvalue[0],'',array('class'=>'formlabel add_settings'));
                    }else{
...
//                         echo $fieldvalue[1];
                        ...
                        if($fieldset_key === 'System Configuration Information' || ($fieldset_key === 'Card Information' && $fieldvalue[0]=='Pricelist')|| ($fieldset_key === 'DID Billing' && $fieldvalue[0]=='Account')|| $fieldset_key === 'Freeswitch Devices' && $fieldvalue[0] == 'Pricelist'){
...
//                    $form_contents.= form_error($fieldvalue[2]['name']);
...
//                                        echo form_error($fieldvalue[2]['name']);
                } else if ($fieldvalue[1] == 'PASSWORD') {
//                    $form_contents.= form_error('password');
                    if(isset($this->CI->input->post))
                    $fieldvalue[2]['value'] = (!$this->CI->input->post($fieldvalue[2]['name'])) ? @$fieldvalue[2]['value'] : $this->CI->input->post($fieldvalue[2]['name']);
                    else
                    $fieldvalue[2]['value'] = ($values) ? $values[$fieldvalue[2]['name']] : @$fieldvalue[2]['value'];
...
//                    exit;
		    ...
		    $form_contents.= '<span class="error" id="'.$fieldvalue[2]['name'].'_error">&nbsp;</span>';
                    ...
                    if($fieldvalue[3]['value'] == "1"){$checked = true;}else{$checked = false;};                    
...
//                    $form_contents.= form_checkbox($fieldvalue[2], 'on', FALSE);
                }
                $form_contents.= '</li>';
            }
